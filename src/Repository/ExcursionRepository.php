...
/**
 * Hardcoded, in-memory replacement for the database-backed excursion
 * repository. No Doctrine / EntityManager involved: the data below is what
 * used to live in App\DataFixtures\ExcursionFixtures and the excursion
 * DB table.
 */
class ExcursionRepository
{
    /** @var Excursion[]|null */
    private static ?array $excursions = null;
    ...
    public function find($id): ?Excursion
    {
        $id = (int) $id;
        foreach ($this->getExcursions() as $excursion) {
     ...
     * @return Excursion[]
     */
    private function getExcursions(): array
    {
        if (self::$excursions === null) {
            self::$excursions = $this->buildExcursions();
        }
    ...
    private function buildExcursions(): array
    {
        ...
        usort($excursions, fn (Excursion $a, Excursion $b) => $a->getPosition() <=> $b->getPosition());
